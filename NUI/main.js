...
$(function(){
	window.addEventListener('message', function(event) {
		...
		}else if (event.data.action == "updateHungerThirst"){
		$(document).ready(function(){
			var data = event.data;
			$("#boxHunger").css("width", data.hunger + "%");
			$("#boxThirst").css("width", data.thirst + "%");
		});
...
function updateStatus(status){
	...
	$('#boxHunger').setValue = "123"
    $('#water').innerHTML = "123"
    $('#boxDrunk').innerHTML = "123"
}
