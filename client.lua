...
AddEventHandler("esx_customui:updateStatus", function (basics)
    --SendNUIMessage({action = "updateHungerThirst", show = true})
	hunger, thirst, drunk = tostring(math.ceil(basics[1].percent)), tostring(math.ceil(basics[2].percent)), tostring(math.ceil(basics[3].percent))
	SendNUIMessage({action = "setValue", key = "hunger", value = hunger.."%"})
	SendNUIMessage({action = "setValue", key = "water", value = thirst.."%"})
	SendNUIMessage({action = "setValue", key = "drunk", value = drunk.."%"})
end)
	...
	while true do
		...
		if isTalking == false then
			...
			if IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
				...
				local essence = exports["LegacyFuel"]:GetFuel(vehicle)
				local vehhealthbefore=math.ceil(GetVehicleBodyHealth(vehicle))-900
				local vehhealth = tostring(vehhealthbefore)
				local speed = GetEntitySpeed(vehicle)
				kmh = tostring(math.ceil(speed * 3.6))
				SendNUIMessage({action = "setValue", key = "gas", value = tostring(math.ceil(essence)).."%"})
				SendNUIMessage({action = "setValue", key = "speed", value = kmh.."kmh"})
				SendNUIMessage({action = "setValue", key = "damage", value = vehhealth.."%"})
			else
				inVehicle = false
